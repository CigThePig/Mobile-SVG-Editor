...
- **[CORE]** Render `<foreignObject>` elements correctly, allowing embedded HTML/CSS content to be displayed and positioned within the SVG coordinate space
- **[STANDARD]** Preserve `<script>` tags and inline DOM event attributes (e.g. `onclick`, `onmouseover`) without stripping them during edits or saves
- **[QOL]** Interactive preview mode that safely executes embedded `<script>` tags and animations to test SVG interactivity directly in the editor
...
- **[QOL]** Support for `<view>` elements to define and name specific `viewBox` targets, enabling native SVG sprite sheet workflows
...
- **[STANDARD]** Non-destructive Boolean operations (compound paths) that render the merged result while retaining the original overlapping shapes for later adjustment
...
- **[STANDARD]** Raster tracing / vectorization tool to convert imported bitmap images into editable SVG paths with threshold and detail controls
- **[QOL]** Shape blending / interpolation tool to automatically generate a specified number of intermediate paths between two distinct shapes
...
- **[STANDARD]** Variable font support with dedicated sliders for `font-variation-settings` (e.g. weight, width, slant, and custom axes)
- **[QOL]** Advanced OpenType feature controls for ligatures, stylistic sets, and tabular numerals mapped to `font-feature-settings`
...
- **[QOL]** Support for parsing the `<color-profile>` element and embedding ICC profiles for accurate print workflows
- **[QOL]** Spot color definition and retention (e.g. Pantone or custom named swatches) for workflows involving vinyl cutters, laser engravers, or print shops
